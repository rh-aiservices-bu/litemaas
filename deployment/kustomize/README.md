...
> **Alternative**: For a Helm-based deployment that works on both vanilla Kubernetes and OpenShift, see [`deployment/helm/litemaas/`](../helm/litemaas/) and the [Helm Deployment Guide](../../docs/deployment/helm-deployment.md).

## Prerequisites

1. **OpenShift cluster access** with project creation permissions
2. **OpenShift CLI (oc)** installed and logged in
3. **OAuth client configured** in OpenShift (see [full deployment guide](../../docs/deployment/kustomize-deployment.md#oauth-client-configuration))
4. **OpenShift groups configured** for role-based access control:
   ...
   See [User Role Management](../../docs/deployment/kustomize-deployment.md#user-role-management-and-openshift-groups) for setup instructions.
...
📚 **Complete Guide**: For detailed setup instructions, see [Kustomize Deployment Guide](../../docs/deployment/kustomize-deployment.md)
