...
test.describe('Subscriptions Page', () => {
  ...
  test('should display subscriptions page with user subscriptions', async ({ page }) => {
    ...
    const hasSubscriptions = await page.getByText('GPT-4').isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('should allow filtering subscriptions by status', async ({ page }) => {
    ...
    const statusFilter = page.getByText('All Statuses').or(page.getByRole('button', { name: /status/i }));
    if (await statusFilter.isVisible()) {
      ...
      const subscriptionCards = page.locator('[data-testid="subscription-card"]').or(
        page.locator('.pf-v6-c-card').filter({ hasText: 'GPT' })
      );
  ...
  test('should open subscription details modal', async ({ page }) => {
    ...
    if (hasSubscriptions) {
      ...
      // Close modal
      await page.getByText('Close').or(page.getByRole('button', { name: 'Close' })).click();
  ...
  test('should allow quota updates', async ({ page }) => {
    ...
    if (hasSubscriptions) {
      ...
      const updateButton = page.getByText('Update Quotas').or(page.getByRole('button', { name: /update/i }));
      if (await updateButton.isVisible()) {
        ...
        // Save changes
        await page.getByText('Save Changes').or(page.getByRole('button', { name: 'Save' })).click();
  ...
  test('should handle subscription cancellation', async ({ page }) => {
    ...
    if (hasSubscriptions) {
      ...
      const cancelButton = page.getByText('Cancel Subscription').or(
      ...
      if (await cancelButton.isVisible()) {
        ...
        const noButton = page.getByText('No, Keep Subscription').or(
          page.getByRole('button', { name: /no|cancel/i })
        );
  ...
  test('should navigate to create subscription from empty state', async ({ page }) => {
    ...
    const isEmptyState = await page.getByText('No subscriptions found').isVisible({ timeout: 3000 }).catch(() => false);
  ...
  test('should show usage warnings for high utilization', async ({ page }) => {
    ...
    if (hasSubscriptions) {
      ...
      const hasWarning = await Promise.all(
        highUsageIndicators.map(indicator => 
